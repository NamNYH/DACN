...
function routes_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	// đảm bảo tính toàn vẹn của tham chiếu ...
	global $Translation;
	$selected_id=makeSafe($selected_id);

	// mm: thành viên có thể xóa bản ghi?
	$arrPerm=getTablePermissions('routes');
	...
	if(($arrPerm[4]==1 && $ownerMemberID==getLoggedMemberID()) || ($arrPerm[4]==2 && $ownerGroupID==getLoggedGroupID()) || $arrPerm[4]==3){ // allow delete?
		// được phép xóa, vì vậy hãy tiếp tục ...
	}else{
...
function routes_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	// hàm trả về biểu mẫu có thể chỉnh sửa cho bản ghi bảng
// và điền vào nó với dữ liệu của bản ghi có ID là $ select_id. Nếu $ select_id
// trống, một biểu mẫu trống được hiển thị, chỉ có 'Thêm mới'
// nút hiển thị.
	...
	// điền bộ lọc, bắt đầu từ trẻ em đến ông bà 
	// giá trị nhận dạng ngẫu nhiên duy nhất
	...
	// mã cho biểu mẫu xem chi tiết dựa trên mẫu
	...
	// đặt bản ghi thành chỉ đọc nếu người dùng không thể chèn bản ghi mới và không thể chỉnh sửa bản ghi hiện tại
	...
	/* mảng trường tra cứu: 'tên trường tra cứu' => mảng ('tên bảng mẹ', 'chú thích trường tra cứu') */
	$lookup_fields = array();
	foreach($lookup_fields as $luf => $ptfc){
		$pt_perm = getTablePermissions($ptfc[0]);

		
		// xử lý các liên kết khóa ngoại
		if($pt_perm['view'] || $pt_perm['edit']){
			$templateCode = str_replace("<%%PLINK({$luf})%%>", '<button type="button" class="btn btn-default view_parent hspacer-md" id="' . $ptfc[0] . '_view_parent" title="' . html_attr($Translation['View'] . ' ' . $ptfc[1]) . '"><i class="glyphicon glyphicon-eye-open"></i></button>', $templateCode);
		}

		
		// nếu người dùng có quyền chèn vào bảng mẹ của trường tra cứu, hãy đặt một nút thêm mới
